...
let PrometheusService = PrometheusService_1 = class PrometheusService {
    constructor(configService) {
        ...
        this.logger = new Logger(PrometheusService_1.name);
        // Counter metrics
        this.httpRequestsTotal = new Map();
        this.httpRequestsByMethod = new Map();
        this.httpRequestsByStatus = new Map();
        this.httpErrorsTotal = new Map();
        // Gauge metrics
        this.httpRequestsInProgress = 0;
        this.activeSessionCount = 0;
        this.databaseConnectionsActive = 0;
        this.githubApiRateLimitRemaining = 5000;
        ...
        this.githubApiCallsRemaining = 5000;
        this.githubApiResetTime = 0;
    }
    /**
     * Record HTTP request start
     */
    recordHttpRequestStart(method, path) {
     ...
     */
    recordHttpRequestEnd(method, path, status, responseTimeMs) {
        this.httpRequestsInProgress--;
     ...
     */
    recordDatabaseQuery(queryTimeMs) {
     ...
     */
    recordGithubApiCall(latencyMs, remaining, resetTime) {
    ...
    export() {
        ...
        lines.push(`http_response_time_ms{quantile="0.5"} ${this.calculatePercentile(this.httpResponseTimeMs, 50)}`);
        lines.push(`http_response_time_ms{quantile="0.95"} ${this.calculatePercentile(this.httpResponseTimeMs, 95)}`);
        lines.push(`http_response_time_ms{quantile="0.99"} ${this.calculatePercentile(this.httpResponseTimeMs, 99)}`);
        ...
        lines.push(`db_query_time_ms{quantile="0.5"} ${this.calculatePercentile(this.databaseQueryTimeMs, 50)}`);
        lines.push(`db_query_time_ms{quantile="0.95"} ${this.calculatePercentile(this.databaseQueryTimeMs, 95)}`);
        lines.push(`db_query_time_ms{quantile="0.99"} ${this.calculatePercentile(this.databaseQueryTimeMs, 99)}`);
        ...
        lines.push(`github_api_latency_ms{quantile="0.5"} ${this.calculatePercentile(this.githubApiLatencyMs, 50)}`);
        lines.push(`github_api_latency_ms{quantile="0.95"} ${this.calculatePercentile(this.githubApiLatencyMs, 95)}`);
        lines.push(`github_api_latency_ms{quantile="0.99"} ${this.calculatePercentile(this.githubApiLatencyMs, 99)}`);
        return lines.join('\n');
    }
    /**
...
PrometheusService = PrometheusService_1 = __decorate([
    Injectable()
], PrometheusService);
