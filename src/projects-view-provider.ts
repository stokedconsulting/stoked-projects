...
export class ProjectsViewProvider implements vscode.WebviewViewProvider {
    ...
    private async loadData(owner: string, repo: string) {
        ...
        console.log(`[gh-projects] Fetched ${repoProjects.length} repo projects and ${allOrgProjects.length} org projects`);
        console.log(`[gh-projects] Repo projects: ${repoProjects.map(p => `#${p.number}`).join(', ')}`);
        console.log(`[gh-projects] Org projects: ${allOrgProjects.map(p => `#${p.number}`).join(', ')}`);
        ...
        console.log(`[gh-projects] Unique org projects (not in repo): ${uniqueOrgProjects.map(p => `#${p.number}`).join(', ')}`);
