...
suite('BranchManager Test Suite', () => {
     ...
     */
    async function setupMockRemote() {
    ...
    setup(async () => {
        ...
        await initGitRepo();
        branchManager = new branch_manager_1.BranchManager(tempDir);
    });
    teardown(() => {
        ...
        (0, branch_manager_1.cleanupBranchManager)();
    });
    ...
    test('pushAgentBranch fails if not on correct branch', async function () {
        ...
        await assert.rejects(async () => await branchManager.pushAgentBranch(agentId, issueNumber), /Not on branch/, 'Should fail when not on the correct branch');
        // Cleanup remote
        fs.rmSync(remotePath, { recursive: true, force: true });
    });
    // ========================================================================
    ...
    test('AC-2.3.f: Auto-rebase aborts on failure, no force-push', async function () {
        ...
        const { stdout: remoteBefore } = await execAsync(`git ls-remote origin agent-${agentId}/project-${issueNumber}`, { cwd: tempDir });
        ...
        const { stdout: remoteAfter } = await execAsync(`git ls-remote origin agent-${agentId}/project-${issueNumber}`, { cwd: tempDir });
    ...
    test('initializeBranchManager creates global instance', () => {
        const manager = (0, branch_manager_1.initializeBranchManager)(tempDir);
        assert.ok(manager instanceof branch_manager_1.BranchManager);
        const retrieved = (0, branch_manager_1.getBranchManager)();
        assert.strictEqual(manager, retrieved, 'Should return same instance');
        (0, branch_manager_1.cleanupBranchManager)();
    });
    test('getBranchManager throws error when not initialized', () => {
        (0, branch_manager_1.cleanupBranchManager)();
        assert.throws(() => (0, branch_manager_1.getBranchManager)(), /not been initialized/, 'Should throw error when manager not initialized');
    });
    test('cleanupBranchManager clears global instance', () => {
        (0, branch_manager_1.initializeBranchManager)(tempDir);
        (0, branch_manager_1.cleanupBranchManager)();
        assert.throws(() => (0, branch_manager_1.getBranchManager)(), /not been initialized/, 'Should throw error after cleanup');
    });
});
