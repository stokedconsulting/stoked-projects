...
suite('HealthMonitor Test Suite', () => {
    ...
    let mockAgentHealthStatuses;
    let mockStuckAgents;
    let mockQueueDepth;
    let mockAgentSessions;
    let mockActiveClaims;
    setup(() => {
        // Create test directory
        testDir = path.join(__dirname, 'test-health-monitor-' + Date.now());
        fs.mkdirSync(testDir, { recursive: true });
        // Initialize managers
        sessionManager = new agent_session_manager_1.AgentSessionManager(testDir);
        // Create mock GitHub API (minimal implementation)
        const mockGitHubApi = {
            getProjectItems: async () => []
        };
        queueManager = new project_queue_manager_1.ProjectQueueManager(testDir, mockGitHubApi);
        ...
        loopValidator = new loop_validator_1.LoopValidator(testDir, sessionManager, queueManager, mockReviewQueueManager);
        ...
        healthMonitor = new health_monitor_1.HealthMonitor(heartbeatManager, loopValidator, queueManager, sessionManager);
        // Mock methods
    ...
    test('AC-5.2.b: Unresponsive agent triggers error notification', async () => {
        ...
        const unresponsiveAlerts = result.alerts.filter(a => a.level === 'error' && a.source === 'agent-health');
        assert.strictEqual(unresponsiveAlerts.length, 1, 'Should have one unresponsive agent alert');
        assert.ok(unresponsiveAlerts[0].message.includes('unresponsive'), 'Alert message should mention unresponsive');
    });
    test('AC-5.2.c: High error rate triggers error notification', async () => {
        ...
        const result = await healthMonitor.runHealthCheck();
        const errorRateAlerts = result.alerts.filter(a => a.level === 'error' && a.source === 'agent-error-rate');
        assert.ok(errorRateAlerts.length > 0, 'Should have error rate alerts');
        assert.ok(errorRateAlerts[0].message.includes('errors in the last hour'), 'Alert should mention error count');
    });
    test('AC-5.2.d: Budget at 90% triggers critical notification and pauses claims', async () => {
        ...
        await (0, cost_tracker_1.logApiUsage)('agent-1', {
        ...
        assert.ok(result.budgetHealth.dailyPercentUsed >= 90, 'Daily budget percent should be >= 90%');
        const budgetAlerts = result.alerts.filter(a => a.level === 'critical' && a.source === 'budget');
        assert.ok(budgetAlerts.length > 0, 'Should have critical budget alert');
        assert.ok(budgetAlerts[0].message.includes('paused'), 'Alert should mention claims being paused');
        assert.strictEqual(healthMonitor.isBudgetPaused(), true, 'Budget should pause new claims');
    });
    test('AC-5.2.e: Stuck agent triggers error notification with restart action', async () => {
        ...
        const result = await healthMonitor.runHealthCheck();
        const stuckAlerts = result.alerts.filter(a => a.level === 'error' && a.source === 'agent-stuck');
        assert.strictEqual(stuckAlerts.length, 1, 'Should have one stuck agent alert');
        assert.ok(stuckAlerts[0].message.includes('stuck') && stuckAlerts[0].message.includes('45 minutes'), 'Alert should mention stuck duration');
    });
    test('AC-5.2.f: Health check failure is logged and skipped gracefully', async () => {
        ...
        const errorAlerts = result.alerts.filter(a => a.level === 'error' && a.source === 'health-check');
        assert.ok(errorAlerts.length > 0, 'Should have health check error alert');
        assert.ok(errorAlerts[0].message.includes('Health check failed'), 'Alert should mention health check failure');
    });
    ...
    test('Queue depth high (> 10) triggers warning alert', async () => {
        ...
        const queueAlerts = result.alerts.filter(a => a.level === 'warning' && a.source === 'queue-depth');
    ...
    test('Start and stop health monitoring', async () => {
        ...
        assert.strictEqual(checkCount, countAfterStop, 'No additional checks should run after stop');
    });
    ...
    test('Overall status correctly reflects highest severity alert', async () => {
        ...
        assert.strictEqual(result.overallStatus, 'warning', 'Overall status should reflect highest severity');
    });
