...
suite('Quality Checks Test Suite', () => {
    ...
    function cleanupTempWorkspace(dir: string) {
    ...
    suite('Test Execution', () => {
        ...
        test('AC-3.4.a: Tests respect timeout limit', async () => {
            ...
            const startTime = Date.now();
            const result = await runTests(testDir, 2000); // 2 second timeout
            const duration = Date.now() - startTime;
    ...
    suite('Linting', () => {
        test('AC-3.4.b: Linting runs and captures error count', async () => {
            ...
            const result = await runLinting(testDir);
    ...
    suite('Security Scanning', () => {
        test('AC-3.4.c: Security scan categorizes vulnerabilities by severity', async () => {
            ...
            const result = await runSecurityScan(testDir);
        ...
        test('AC-3.4.e: Critical or high vulnerabilities fail security check', () => {
            const resultWithCritical: SecurityResult = {
                passed: false,
                vulnerabilities: {
                    critical: 1,
                    high: 0,
            ...
            const resultWithHigh: SecurityResult = {
                passed: false,
                vulnerabilities: {
                    critical: 0,
                    high: 2,
            ...
            const resultWithModerate: SecurityResult = {
                passed: true,
                vulnerabilities: {
                    critical: 0,
                    high: 0,
...
`;
            ...
            const result = await checkDocumentation([filePath]);
...
`;
            ...
            assert.ok(result.undocumented.some(item => item.includes('undocumentedFunction')),
                'Should include undocumented function');
...
`;
            ...
            const result = await checkDocumentation([testFilePath]);
...
`;
    ...
    suite('Overall Quality Checks', () => {
        test('AC-3.4.e: All blocking checks passing results in overall pass', async () => {
            ...
            const result = await runAllQualityChecks(testDir, []);
        ...
        test('AC-3.4.e: Failed tests cause overall failure', async () => {
            ...
            assert.strictEqual(isBlockingFailure(result), true, 'Should be blocking failure');
...
`;
            ...
            const result = await runAllQualityChecks(testDir, [filePath]);
            ...
            assert.strictEqual(isBlockingFailure(result), false, 'Should not be blocking failure');
        ...
        test('isBlockingFailure returns true for test failures', () => {
            ...
            assert.strictEqual(isBlockingFailure(result), true, 'Failed tests should be blocking');
        });
        ...
        test('isBlockingFailure returns true for linting failures', () => {
            ...
            assert.strictEqual(isBlockingFailure(result), true, 'Failed linting should be blocking');
        });
        ...
        test('isBlockingFailure returns true for security failures', () => {
            ...
            assert.strictEqual(isBlockingFailure(result), true, 'Failed security should be blocking');
        });
        ...
        test('isBlockingFailure returns false for documentation-only failures', () => {
            ...
            assert.strictEqual(isBlockingFailure(result), false, 'Failed documentation should not be blocking');
        });
    });
