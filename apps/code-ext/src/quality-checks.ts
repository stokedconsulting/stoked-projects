...
/**
 * Result of documentation coverage check
 */
 ...
 */
function detectPackageManager(workspaceRoot: string): 'npm' | 'pnpm' {
 ...
 */
export async function runTests(workspaceRoot: string, timeout: number = 300000): Promise<TestResult> {
 ...
 */
export async function runLinting(workspaceRoot: string, files: string[] = []): Promise<LintResult> {
    let errorCount = 0;
    let warningCount = 0;
    let errors: Array<{file: string; line: number; message: string}> = [];
    let passed = false;
 ...
 */
export async function runSecurityScan(workspaceRoot: string): Promise<SecurityResult> {
    let vulnerabilities = {
        critical: 0,
        high: 0,
        moderate: 0,
 ...
 */
export async function checkDocumentation(files: string[]): Promise<DocumentationResult> {
 ...
 */
export function isBlockingFailure(result: QualityCheckResult): boolean {
